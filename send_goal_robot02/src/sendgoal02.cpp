...
void vreadCSVfile(void)
{
    ifstream MyReadFile1("/home/SharedFile/sendgoal02.csv");
    if(!MyReadFile1.is_open())
			cout<<"FILE SENDGOAl ROBOT2 CAN'T OPEN PLEASE CHECK FILE LOCATION"<<endl;
...
void vmoveToposition(double dx,double dy,double dtheta)
{
    MoveBaseClient ac("tb3_1/move_base",true);
		...
		{
			ofstream writefile;
			writefile.open("/home/SharedFile/rfid02.csv");
			writefile<<"id"<<"\n";
			writefile<<"456";
			writefile.close();
...
{	
	ros::init(argc,argv,"Robot02_From_CSV");
	ros::NodeHandle nh;
	while(nh.ok())
       {
 		vreadCSVfile();
