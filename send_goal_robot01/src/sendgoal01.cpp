...
double vconvertstringTodoubleOfx(string sx);
double vconvertstringTodoubleOfy(string sy);
...
void vreadCSVfile(void)
{
    ifstream MyReadFile1("/home/SharedFile/sendgoal01.csv");
    if(!MyReadFile1.is_open())
			cout<<"FILE SENDGOAl ROBOT1 CAN'T OPEN PLEASE CHECK FILE LOCATION"<<endl;
	...
	while(getline(MyReadFile1,line))
	{
        ...
        dx[1]=vconvertstringTodoubleOfx(str_x[icount]);
        dy[1]=vconvertstringTodoubleOfy(str_y[icount]);
		icount++;
...
void vmoveToposition(double dx,double dy,double dtheta)
{
    MoveBaseClient ac("tb3_0/move_base",true);
		...
		{
			//ofstream writefile;
			//writefile.open("/home/SharedFile/rfid01.csv");
			//writefile<<"id"<<"\n";
			//writefile<<"123";
			//writefile.close();
...
{	
	ros::init(argc,argv,"Robot01_From_CSV");
	ros::NodeHandle nh;
	while(nh.ok())
       {
 		vreadCSVfile();
